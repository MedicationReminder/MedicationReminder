...
/*
 * Created by Xu Wei on 2017/5/03.
 * 网络请求工具类XUtilHttpRequest
 */
public  class XUtilHttpRequest {
    private volatile static XUtilHttpRequest instance;
    ...
    public static XUtilHttpRequest getInstance() {
        if (instance == null) {
            synchronized (XUtilHttpRequest.class) {
                if (instance == null) {
                    instance = new XUtilHttpRequest();
                }
            }
        }
        return instance;
    ...
    private void onFailResponse(final String s, final XCallBack callBack) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                if (callBack != null) {
